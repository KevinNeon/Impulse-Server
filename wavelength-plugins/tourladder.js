...
WL.updateTourLadder = function (p1, p2, result, room) {
	WL.tourLadder.all("SELECT * FROM tourladder WHERE userid = '" + p1.userid + "' OR userid = '" + p2.userid + "';", (err, users) => {
		...
		WL.tourLadder.run(sql1, () => {
			WL.tourLadder.run(sql2, () => {
				...
				room.addRaw('<br />' + WL.nameColor(p1.name, true) + '\'s rating: ' + Math.round(p1elo) + ' &rarr; <strong>' + Math.round(newP1Elo) + '</strong><br />(' + reasons + ')');

				reasons = '' + (Math.round(newP2Elo) - Math.round(p2elo)) + ' for ' + (result === 'win' ? 'losing' : (result === 'loss' ? 'winning' : 'tying'));
				if (reasons.charAt(0) !== '-') reasons = '+' + reasons;
				room.addRaw(WL.nameColor(p2.name, true) + '\'s rating: ' + Math.round(p2elo) + ' &rarr; <strong>' + Math.round(newP2Elo) + '</strong><br />(' + reasons + ')');
...
WL.addTourWin = function (user) {
	WL.tourLadder.run("UPDATE tourladder SET tourwins = tourwins + 1 WHERE userid = '" + toId(user) + "';");
};
...
exports.commands = {
	...
	tourladder: function (target, room, user) {
		...
		if (!target || !target.trim()) {
			WL.tourLadder.all("SELECT * FROM tourladder ORDER BY elo DESC;", (err, users) => {
				...
				for (let i = 0; i < (this.broadcasting && users.length > 10 ? 10 : users.length); i++) {
					...
					table += '<tr><td><center>' + (i + 1) + '</center></td><td style = "text-align: center">' + WL.nameColor(name, true) + '</td><td style = "text-align: center">' + Math.round(users[i].elo) + '</td>' +
		...
		WL.tourLadder.all("SELECT elo FROM tourladder WHERE userid = '" + toId(target) + "';", (err, users) => {
			if (err) return console.log("/tourladder 2: " + err);
			if (!users.length) {
				this.sendReplyBox(WL.nameColor(target, true) + ' has not played any rated tournaments yet.');
			} else {
				this.sendReplyBox(WL.nameColor(target, true) + '\'s Tournament Elo is <b>' + Math.round(users[0].elo) + '</b>.');
			}
			room.update();
		});
	},
	...
	tourladderusers: function (target, room, user) {
		if (!this.runBroadcast()) return;
		WL.tourLadder.all("SELECT COUNT(*) FROM tourladder;", (err, count) => {
	...
	resettourladder: function (target, room, user) {
		...
		WL.tourLadder.all("SELECT * FROM tourladder;", (err, users) => {
			...
			WL.tourLadder.run("DELETE FROM tourladder;", () => {
